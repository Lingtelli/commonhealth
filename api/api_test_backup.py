...
def isQualified(member, center, author, start, end, source):
    ...
    if ((start is not None) and (end is not None)):
        ...
        if ((datetime.strptime(date, "%Y%m%d").date() < datetime.strptime(start, "%Y%m%d").date()) or 
            ...
            return False
        '''
        if ((datetime.strptime(date, "%Y%m%d").date() < datetime.fromtimestamp(int(start)/1000.0).date()) or 
                (datetime.strptime(date, "%Y%m%d").date() > datetime.fromtimestamp(int(end)/1000.0).date())):
            return False
        '''
    return True
