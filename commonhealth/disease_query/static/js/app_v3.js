(function() {
   ...
   app.controller("TypeController", function($scope, Data) {
      ...
      function initialTypes(_disease_clusters) {
         ...
         $scope.titles = Array.from(titles);
      ...
      $scope.generateQueryResults = function(keyword, clusterid) {
         console.log(keyword + ' , ' + clusterid)
         console.log($scope.disease_name + ' , ' + $scope.query_keyword + ' , ' + $scope.query_clusterid);
         if (keyword != null)
            $scope.query_keyword = keyword;
         if (clusterid != null)
            $scope.query_clusterid = clusterid.substring(2);
         console.log($scope.disease_name + ' , ' + $scope.query_keyword + ' , ' + $scope.query_clusterid);
         ...
         if ($scope.query_clusterid != null) {
            for (var idx = 0; idx < temp_results.length; idx++) {
               if ($scope.query_clusterid == temp_results[idx]['title'].substring(2)) {
                  var cluster = temp_results[idx];
                  for (var jdx in cluster['member']) {
                     if (cluster['member'][jdx]['keywords'].indexOf($scope.query_keyword) == -1) {
            ...
            for (var idx  = 0; idx < temp_results.length; ++idx) {
               if (($scope.query_clusterid != null) && ($scope.query_clusterid != temp_results[idx].substring(2))){
                  delete(temp_results[idx]);
                  console.log(temp_results.length);
                  continue;
               }
               ...
               for (var jdx in cluster['member']) {
                  ...
                  if (cluster['member'][jdx]['author'].indexOf($scope.deselected_types) == -1) {
                     delete(temp_results[idx]['member'][jdx]);
                     continue;
                  }
                  /*
                  // If types of a paragraph are subarray of deselected_types,
                  // delete the paragraph from results.
